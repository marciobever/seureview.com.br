...
export default function Header({ initialLoggedIn = false }: Props) {
  ...
  useEffect(() => {
    const check = async () => {
      try { const r = await fetch('/api/auth/me', { cache: 'no-store' }); setLoggedIn(r.ok); }
  ...
  async function logout() { try { await fetch('/api/auth/logout', { method: 'POST' }); } catch {} setLoggedIn(false); window.location.href = '/'; }
      ...
      <div className="max-container py-3">
        <div className="flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            {navPublic.map((i) => (
              <Link key={i.href} href={i.href} className="nav-item">{i.label}</Link>
            ))}
          ...
          <button
            ...
            onClick={() => setOpen((v) => !v)} aria-expanded={open} aria-controls="mobile-menu" aria-label="Abrir menu"
          >
            {open ? 'Fechar' : 'Menu'}
          </button>
        </div>

        <div id="mobile-menu" className={cx('md:hidden transition-all overflow-hidden', open ? 'max-h-[60vh] mt-3' : 'max-h-0')}>
